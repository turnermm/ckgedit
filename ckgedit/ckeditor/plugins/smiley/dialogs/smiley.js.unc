...
CKEDITOR.dialog.add( 'smiley', function( editor ) {
	...
	var onClick = function( evt ,data) {
            ...
            img_class = target.getAttribute( 'class' );
                     
            if(img_class.match(/doku/)) {
   ...
   var dwimages;
   if(config.dokuSmileyConfImages) {
         dwimages = config.dokuSmileyConfImages;
   }
   else dwimages = config.dokuSmileyImages; 
   
	var size = dwimages.length;
	for ( i = 0; i < size; i++ ) {
		if ( i % columns === 0 )
			html.push( '<tr role="presentation">' );
          
		var smileyLabelId = 'cke_smile_label_' + i + '_' + CKEDITOR.tools.getNextNumber();
      var descr = dwimages[i][0];
       
		html.push( '<td class="cke_dark_background cke_centered" style="vertical-align: middle;" role="presentation">' +
			'<a href="javascript:void(0)" role="option"', ' aria-posinset="' + ( i + 1 ) + '"', ' aria-setsize="' + size + '"', ' aria-labelledby="' + smileyLabelId + '"', ' class="cke_smile cke_hand" tabindex="-1" onkeydown="CKEDITOR.tools.callFunction( ', onKeydown, ', event, this );">', '<img class="cke_hand doku" title="', descr, '"' +
					' cke_src="', CKEDITOR.tools.htmlEncode( config.dokuSmileyPath + dwimages[ i ][1] ), '" alt="', descr, '"', ' src="', 
                    CKEDITOR.tools.htmlEncode( config.dokuSmileyPath + dwimages[ i ][1] ), '"',
		// IE BUG: Below is a workaround to an IE image loading bug to ensure the image sizes are correct.
		( CKEDITOR.env.ie ? ' onload="this.setAttribute(\'width\', 2); this.removeAttribute(\'width\');" ' : '' ), '>' +
				'<span id="' + smileyLabelId + '" class="cke_voice_label">' + descr + '</span>' +
			'</a>', '</td>' );
	...
	if ( i < columns - 1 ) {
       ...
       html.push('<tr>');
       dwimages = config.dokuFixmeSmiley;
       for(i=0; i< dwimages.length; i++) {
		html.push( '<td colspan="4" class="cke_dark_background cke_centered" style="vertical-align: middle;" role="presentation">' +
			'<a href="javascript:void(0)" role="option"', ' aria-posinset="' + ( i + 1 ) + '"', ' aria-setsize="' + size + '"', ' aria-labelledby="' + smileyLabelId + '"', ' class="cke_smile cke_hand" tabindex="-1" onkeydown="CKEDITOR.tools.callFunction( ', onKeydown, ', event, this );">', '<img class="cke_hand doku" title="', descr, '"' +
					' cke_src="', CKEDITOR.tools.htmlEncode( config.dokuSmileyPath + dwimages[ i ][1] ), '" alt="', descr, '"', ' src="', 
                    CKEDITOR.tools.htmlEncode( config.dokuSmileyPath + dwimages[ i ][1] ), '"',
		// IE BUG: Below is a workaround to an IE image loading bug to ensure the image sizes are correct.
