function parse_wikitext(id) {
 ...
 var line_break = "\nL_BR_K  \n";
    ...
    var markup_end = { 'del': '</del>', 's': '</del>', 'strike': '</del>', 'p': " ", 'br':" ", 'a': ']]','img': '\}\}',
          ...
          'td': " ", 'th': " ", 'tr':"|\n", 'ol':" ", 'ul': " ", 'li': "\n",
           'pre': "\n</",'sub': '</sub>', 'sup': '</sup> ', 'div':"\n\n", 'p': "\n\n",
           'font': "</font>", 'span': " ",'blockquote': '</blockquote>',
     }; 
  ...
  //             if(!confirm(attrs[i].name + '="' + attrs[i].escaped + '"')) exit;
	...
	if(this.interwiki && results.match(/>\w+\s*\|$/)) 	{	 
     ...
     text = text.replace(/&not;ags/g, '&notags'); //replace &not entity in include notags param	 
...
results = results.replace(/(\s*={2,})(.*?)(\[\[|\{\{)(.*?)(\]\]|\}\})(.*?)\1/gm, function(m,h_markup,whatever,bracket_1,inner,bracket_2, end_str) { 
	...
	results = results.replace(/\{ \{ rss&gt;sFeed:/mg,'{{rss&gt;https://')
    ...
    if(HTMLParser_FORMAT_SPACE) { 
     ...
     /*fix for links in lists at ends of lines, which cause loss of line-feeds 
         ...
         /*fix for links in lists before ends of lines, which cause extra  line-feeds */
            results = results.replace(  /(\*|-).*?(\[\[|\{\{).*?(\]\]|\}\})([\s\w\/\-\x3A-\x40\x5B-\x60\x7B-\x7F,;\>\<\&]+)\n\n/mg,
                 function(all,type,b,c,tail) {    
                  all = all.replace(/[\n]$/,"");                       
			      return (all);
           }); 
...
jQuery( "#ebut_cancel" ).mouseup(function() { 
   if( this.form.template && this.form.template.value == 'tpl' ) return;
   if(window.dwfckTextChanged) return;
   draft_delete();
});
