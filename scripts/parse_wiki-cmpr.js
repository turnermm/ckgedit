 ...
 var line_break = "\nL_BR_K  \n";
   ...
   */
    ...
    results = results.replace(/%%\\%%/g,'_ESC_BKSLASH_');
     ...
     results = results.replace(/(\s*={2,})\s*CKGE_TMP_(\w+)(.*?)\s*CKGE_TMP_c?\2/gm, function(m,tag) {   //remove formats from headers
   ...
   // fix for colspans which have had text formatting which cause extra empty cells to be created
    ...
    if(HTMLParserFont) {
       ...
       if(HTMLParserFontInfix) {
        ...
        results = results.replace(/(<font[^\>]+>)([^<]+(\]\])?)[^\>]+\/font>\s*(\]\])/gm, function(match,a,b) {
            b=b.replace(/\n/gm,"");
            b=b.replace(/\s/gm,"");
            b=b.replace(/[\[\]]/g,"");
            b = '[[' + b + ']]';
